...
first_row:          li $t3, 0x01                            # check all row
polling:            sb $t3, 0($t1 )                         # must reassign expected row pected row 
                    lb $a0, 0($t2)                          # read scan code of key button f key button 
                    beqz $a0, check_next_row                # if no key pressed, check next rowexa) 
        print:      li $v0, 34                              # print integer (hexa) 
                    syscall 
        sleep:      li $a0, 100                             # sleep 100ms 
                    li $v0, 32 
                    syscall 
check_next_row:     bge $t3, 0x08, first_row                # if all row checked, check first row
                    sll $t3, $t3, 1                         # check next row 
